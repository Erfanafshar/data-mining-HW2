...
tf = pd.read_csv("titanic\\test.csv")
tf = normalization(tf, False)
predicts = clf.predict(tf)
print(predicts)

out = pd.DataFrame({"PassengerId": list(range(892, 1310)),
                    "Survived": predicts})

csv_out = out.to_csv("output.csv", index=False)
